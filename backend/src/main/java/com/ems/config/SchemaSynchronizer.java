...
public class SchemaSynchronizer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        try {
            // Leave Types Schema
            jdbcTemplate
                    .execute("ALTER TABLE leave_types ADD COLUMN IF NOT EXISTS color VARCHAR(255) DEFAULT '#34d399';");
            jdbcTemplate.execute("UPDATE leave_types SET color = '#34d399' WHERE color IS NULL;");

            jdbcTemplate
                    .execute("ALTER TABLE leave_types ADD COLUMN IF NOT EXISTS require_approval BOOLEAN DEFAULT true;");
            jdbcTemplate.execute("UPDATE leave_types SET require_approval = true WHERE require_approval IS NULL;");

            jdbcTemplate.execute(
                    "ALTER TABLE leave_types ADD COLUMN IF NOT EXISTS require_attachment BOOLEAN DEFAULT false;");
            ...
            jdbcTemplate.execute(
                    "ALTER TABLE leave_types ADD COLUMN IF NOT EXISTS applicable_after_months INTEGER DEFAULT 0;");
            jdbcTemplate.execute(
                    "UPDATE leave_types SET applicable_after_months = 0 WHERE applicable_after_months IS NULL;");

            // Holidays Schema
            jdbcTemplate
                    .execute("ALTER TABLE holidays ADD COLUMN IF NOT EXISTS type VARCHAR(255) DEFAULT 'GOVERNMENT';");
            ...
            logger.info("✅ Schema: Synchronized Core Columns, Notifications, and Patched NULLs.");
        } catch (Exception e) {
            logger.warn("⚠️ Schema Sync Notice: {}", e.getMessage(), e);
        }
    }
}
