package com.ems.controller.Handbook;
...
public class AdminHandbookController {
    ...
    @PostMapping("/policy/{id}/document")
    public ResponseEntity<HandbookPolicyResponse> uploadPolicyDocument(
            @PathVariable("id") Long id,
            @RequestPart("file") org.springframework.web.multipart.MultipartFile file,
            @RequestHeader("Authorization") String token) {

        Long hrEmployeeId = jwtUtil.extractUserId(token.substring(7));
        HandbookPolicyResponse resp = handbookService.uploadPolicyDocument(id, file, hrEmployeeId);
