...
public class LeaveServiceImpl implements LeaveService {
    ...
    @Transactional
    public void sweepOutdatedLeaves() {
        leaveRequestRepository.markOutdatedPendingLeaves(LocalDate.now());
    }
    ...
    @Override
    @Transactional
    public LeaveRequestResponse modifyLeaveRequest(Long leaveId, Long employeeId, LeaveModifyRequest request) {
        LeaveRequest leaveRequest = leaveRequestRepository.findById(leaveId)
                .orElseThrow(() -> new ResourceNotFoundException("Leave request not found."));
        Employee employee = getEmployeeOrThrow(employeeId);

        if (!leaveRequest.getEmployee().getId().equals(employee.getId())) {
            throw new IllegalArgumentException("You can only modify your own leave requests.");
        }
        ...
        leaveRequest.setActionStatus("MODIFY_REQUESTED");
        leaveRequest.setProposedStartDate(request.getProposedStartDate());
    ...
    @Override
    @Transactional
    public LeaveRequestResponse approveLeaveAction(Long leaveId, Long managerId) {
        LeaveRequest leaveRequest = leaveRequestRepository.findById(leaveId)
                .orElseThrow(() -> new ResourceNotFoundException("Leave request not found."));
        Employee manager = getEmployeeOrThrow(managerId);
        Employee employee = leaveRequest.getEmployee();
        ...
        if ("CANCEL_REQUESTED".equalsIgnoreCase(leaveRequest.getActionStatus())) {
        ...
        } else if ("MODIFY_REQUESTED".equalsIgnoreCase(leaveRequest.getActionStatus())) {
            boolean wasApproved = "Approved".equalsIgnoreCase(leaveRequest.getStatus());
            ...
            leaveRequest.setStartDate(leaveRequest.getProposedStartDate());
            leaveRequest.setEndDate(leaveRequest.getProposedEndDate());
            leaveRequest.setTotalDays(leaveRequest.getProposedTotalDays());
            leaveRequest.setReason(leaveRequest.getProposedReason());
            leaveRequest.setActionStatus("NONE");
            ...
            if (wasApproved) {
                // Recompute LOP split and create new attendance
                LeaveAllocationSnapshot allocation = calculateLeaveAllocation(
                        employee,
                        leaveRequest.getStartDate(),
                        leaveRequest.getEndDate(),
    ...
    private LeaveRequestResponse mapToResponse(LeaveRequest lr) {
        ...
        return LeaveRequestResponse.builder()
                ...
                .actionStatus(lr.getActionStatus())
                .proposedStartDate(lr.getProposedStartDate())
                .proposedEndDate(lr.getProposedEndDate())
                .proposedTotalDays(lr.getProposedTotalDays())
                .proposedReason(lr.getProposedReason())
