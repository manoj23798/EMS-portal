...
import com.ems.service.LeaveAnalyticsService;
...
public class LeaveAnalyticsServiceImpl implements LeaveAnalyticsService {
    ...
    @Override
    public Map<String, Object> getDashboardStats(Long employeeId, Long departmentId, LocalDate startDate, LocalDate endDate) {
        ...
        List<LeaveRequest> filtered = allRequests.stream()
                ...
                .filter(lr -> {
                    ...
                    return emp != null && emp.getDepartment() != null && emp.getDepartment().getId().equals(departmentId);
                })
        ...
        stats.put("totalLeavesTaken", filtered.stream()
                ...
                .mapToInt(lr -> lr.getTotalDays() != null ? lr.getTotalDays() : 0)
                .sum());
        ...
        long other = distribution.entrySet().stream()
                .filter(e -> !e.getKey().contains("Annual") && !e.getKey().contains("Casual") && !e.getKey().contains("Sick"))
    ...
    public List<Map<String, Object>> getLeaveCalendar(LocalDate start, LocalDate end) {
        ...
        try {
            List<LeaveRequest> leaves = leaveRequestRepository.findAll().stream()
                    ...
                    .filter(lr -> "Approved".equalsIgnoreCase(lr.getStatus()) || "Pending".equalsIgnoreCase(lr.getStatus()))
                    .collect(Collectors.toList());
